...
export function GuidanceSearchPanel({ pilgrim, onSelect }: { pilgrim: Pilgrim | null; onSelect: (destination: DestinationType) => void }) {
  ...
  return <section className="grid gap-5 rounded-[2rem] border border-gold-100 bg-white p-6 shadow-card lg:grid-cols-[1fr_300px]">
    <div><div className="flex items-center gap-3"><MapPinned className="h-7 w-7 text-brand-700" /><h2 className="text-kiosk-lg font-bold text-brand-900">{t("ابحث عن وجهتك على الخريطة", "Search for a destination on the map")}</h2></div><div className="mt-4 flex gap-2"><input value={query} onChange={(event) => { setQuery(event.target.value); setSearchError(false); }} onKeyDown={(event) => { if (event.key === "Enter") search(); }} placeholder={t("اكتب اسم الوجهة، مثل المسجد الحرام أو صيدلية", "Enter a destination, such as Grand Mosque or pharmacy")} className="h-touch min-w-0 flex-1 rounded-2xl border-2 border-cream-300 bg-cream-100 px-4 text-kiosk-xs outline-none focus:border-brand-700" /><button type="button" onClick={search} disabled={!query.trim()} className="flex h-touch items-center gap-2 rounded-2xl bg-brand-700 px-5 font-bold text-white disabled:opacity-50"><Search className="h-5 w-5" />{t("بحث", "Search")}</button></div>{searchError && <p className="mt-2 text-sm font-semibold text-emergency">{t("لم نجد الوجهة تجريبيًا. جرّب اسم معلم أو خدمة قريبة.", "Destination not found in the demo. Try a landmark or nearby service.")}</p>}<h3 className="mt-5 font-bold text-brand-900">{t("البحث الأكثر شيوعًا", "Popular searches")}</h3><div className="mt-3 flex flex-wrap gap-3">{POPULAR.map((item) => <button key={item.destination} type="button" onClick={() => onSelect(item.destination)} className="flex min-h-12 items-center gap-2 rounded-2xl border border-cream-300 bg-cream-100 px-4 font-bold text-ink-700 hover:border-gold-400 hover:bg-gold-100"><item.icon className="h-5 w-5 text-brand-700" />{t(item.ar, item.en)}</button>)}</div></div>
